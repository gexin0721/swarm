#ifndef DATA_PROCESSING_H
#define DATA_PROCESSING_H

//  =============================== 头文件调用 ==================
#include <iostream>
#include <string>
#include <vector>
#include <queue>
#include <mutex>
#include <thread>
#include <chrono>
#include <json/json.h>
#include <sstream>      // std::stringstream
#include <algorithm>    // std::min
#include <stdexcept>    // std::runtime_error
#include "./../UDP/UDP.h"
//  ============================= 公共变量声明 ==================
extern SwarmRegistry swarm_registry;

// ============================= 类声明 ==========================
// 无人机参数类
class DataProcessing {
public:
    // PID控制参数类
    struct PID
    {
        float kp = 0.0f;//PID比例系数
        float ki = 0.0f;//PID积分系数
        float kd = 0.0f;//PID微分系数
    };

    uint8_t id = 0;//无人机编号
    // MPU6050数据
    int16_t roll = 0;//横滚角
    int16_t pitch = 0;//俯仰角
    int16_t yaw = 0;//偏航角

    // GPS数据
    float x = 0;//经度
    float y = 0;//纬度
    float z = 0;//高度
    // ADC数据
    uint8_t batt = 0;//电池电压
    // PID数据
    struct PID pid[4] = {0};



    // 更新
    void ParseData(const std::string& data);
    void ParseData(const Json::Value& data);
    void ParseData(const uint8_t* data);
    void ParseData(const std::vector<uint8_t>& data); 

    // 更新 初始化数据
    void Init_ParseData(const std::vector<Json::Value>& data);

};

// 无人机数据

template<typename T>
class DroneData
{
private:
    DataProcessing* data = NULL;
    int size = 0;

    // 获取ID信息
    uint8_t returnID(const std::string& data)
    {
        // 查找第一个键值对中的ID
        size_t pos = data.find("id=");
        if (pos != std::string::npos) {
            // 找到"id="后，提取等号后的值
            size_t start = pos + 3; // "id="的长度是3
            size_t end = data.find(',', start); // 查找下一个逗号
            if (end == std::string::npos) {
                end = data.length(); // 如果没有逗号，取到字符串末尾
            }
            std::string id_str = data.substr(start, end - start);
            try {
                return static_cast<uint8_t>(std::stoi(id_str));
            } catch (const std::exception& e) {
                return 0; // 解析失败返回0
            }
        }
        return 0; // 未找到ID返回0
    }
    uint8_t returnID(const Json::Value& data)
    {
        if (data.isMember("id") && data["id"].isInt()) {
            return static_cast<uint8_t>(data["id"].asInt());
        }
        return 0; // 未找到ID或类型错误返回0
    }
    uint8_t returnID(const uint8_t* data)
    {
        // 直接从数组[2]获取ID号
        if (data != nullptr && data[0] == 0xEE && data[1] == 0xEE) {
            return data[2]; // 状态位就是ID号
        }
        return 0; // 包头错误或数据为空返回0
    }
    
    uint8_t returnID(const std::vector<uint8_t>& data)
    {
        // 直接从数组[2]获取ID号
        if (!data.empty() && data.size() >= 3 && data[0] == 0xEE && data[1] == 0xEE) {
            return data[2]; // 状态位就是ID号
        }
        return 0; // 数据不足或包头错误返回0
    }
public:
    //  =================== 构造函数 ===================
    // 参数一：无人机数量
    DroneData(const uint8_t *id, const int cont = 0)
    {
        if (cont <= 0)
        {
            return;
        }
        this->data = new DataProcessing[cont];//创建无人机数据数组
        for (int i = 0; i < cont; i++)
        {
            this->data[i].id = id[i];
        }

        this->size = cont;
    }
    //  =================== 析构函数 ===================
    ~DroneData()
    {
        if (data != NULL)//释放无人机数据数组
        {
            delete[] data;
            data = NULL;
        }
    }

    //  添加无人机数据
    // 参数一：无人机ID数组
    // 参数二：无人机数量
    void addData(const std::vector<uint8_t>& id, int size)
    {
        if (id == nullptr)
        {
            throw std::runtime_error("DroneData未正确初始化，data指针为空");
        }
        if (size <= 0)
        {
            throw std::runtime_error("DroneData未正确初始化，size为0");
        }
        this->data = new DataProcessing[this->size + size];
        for (int i = 0; i < this->size; i++)
        {
            this->data[i] = id[i];
        }
        this->size += size;
    }

    //  =================== 迭代器 ===================
    DataProcessing* begin()
    {   
        if (data == nullptr) {
            throw std::runtime_error("DroneData未正确初始化，data指针为空");
        }
        return data;
    }
    DataProcessing* end()
    {
        if (data == nullptr) {
            throw std::runtime_error("DroneData未正确初始化，data指针为空");
        }
        return data + size;
    }

    // =================== 运算符 ===================
    DataProcessing& operator[](int index)
    {
        if (data == nullptr) {
            throw std::runtime_error("DroneData未正确初始化，data指针为空");
        }
        if (index < 0 || index >= size) {
            throw std::out_of_range("DroneData下标越界");
        }
        return data[index];
    }
    DataProcessing& operator[](int index) const
    {
        if (data == nullptr) {
            throw std::runtime_error("DroneData未正确初始化，data指针为空");
        }
        if (index < 0 || index >= size) {
            throw std::out_of_range("DroneData下标越界");
        }
        return data[index];
    }

    // =================== 大小 ===================
    int size() const
    {
        return size;
    }
    // =================== 判断是否为空 ===================
    bool empty() const
    {
        return size == 0 && data == nullptr;
    }

    //  =================== 遍历缓存 ===================
    /**
     * @brief 解析消息队列中的数据
     * @param message_cache 待解析的消息队列
     * @throws std::runtime_error 当数据解析失败或data指针为空时
     * @note 模板设计：传什么类型用什么类型，编译时确定调用哪个重载版本
     */
    void ParseData(std::queue<T> &message_cache)
    {
        // 检查data指针是否有效
        if (data == nullptr) {
            throw std::runtime_error("DroneData未正确初始化，data指针为空");
        }

        // 逐个解析队列中的消息
        while (!message_cache.empty())
        {
            try {

                auto Data = message_cache.front();
                uint8_t ID = returnID(Data);
                for (i : size)//遍历数组。防止数据越界
                {
                    if(data[i].id == ID)
                    {
                        data[i].ParseData(Data);
                        break;//跳出循环
                    }
                }
                
                // 解析成功，弹出已处理的消息
                message_cache.pop();
            }
            catch (const std::exception& e) {
                // 单个消息解析失败，抛出具体错误信息
                throw std::runtime_error("数据解析失败: " + std::string(e.what()) + 
                                        " (数据类型可能不匹配)");
            }
        }
    }
};


#endif